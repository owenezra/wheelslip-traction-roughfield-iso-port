...
The grader may own private OpenSeesPy models and fixed analysis cases, but the
agent-facing task must still require a public OpenSeesPy analysis. This keeps
private scoring reproducible while ensuring the task actually exercises
structural solver use.
...
- `[difficulty]`: `task_type = "structures"`, `domain = "seismic_retrofit"`,
  and `reward_type = "multi_deterministic_rubrics"` for the standard
  deterministic engineering-rubric pattern.
