"""Reference policy artifact for the rover traction-programming task.

``load_policy()`` reconstructs the full policy from ``policy.pt`` (a joblib
payload holding the fitted friction-identifier models) next to this module.

Per evaluation episode the policy:

  1. extracts a fixed engineered feature vector from the probe-phase
     observation dict (``window_features``: channel statistics, transmitted-
     force plateau quantiles from an encoder-based force observer built on the
     disclosed motor constants, free-spin fractions, command-conditioned
     accelerometer statistics, spectral band powers, coast-decay slopes, and
     lagged command-response correlations);
  2. predicts the episode's friction coefficient mu_hat as the clipped mean of
     the fitted identifier models' predictions (the checked-in reference
     training recipe fits a deterministic histogram gradient booster);
  3. plans the 14-knot blind-phase torque program with a heuristic-initialized
     restarted cross-entropy-method (CEM) search over open-loop programs,
     simulating candidates on the disclosed slope at mu_hat and scoring them
     with the disclosed stop-and-settle quality metric evaluated at mu_hat.

The module is self-contained at evaluation time: it carries its own copy of
the slope-phase simulator (identical XML and constants to the practice
environment) and reads nothing outside its own directory. Planning is seeded
from the episode's disclosed ``probe_seed``, so rollouts are deterministic.
"""
from __future__ import annotations

import os

# MuJoCo/NumPy/scikit-learn execute on the host CPU. Keep native math
# libraries single-threaded for deterministic grading and to avoid
# oversubscribing the TPU host.
for _name in (
    "OMP_NUM_THREADS",
...
class TractionProgramPolicy:
    """Identify mu from the probe window, then plan the blind program."""
    ...
    def predict_mu(self, observation: dict) -> float:
        F = window_features(observation).reshape(1, -1)
        F = np.nan_to_num(F)
        lo, hi = MU_PHYSICAL
        preds = [float(np.clip(m.predict(F)[0], lo, hi)) for m in self.members]
        return float(np.clip(np.mean(preds), lo, hi))

    def act(self, observation: dict) -> np.ndarray:
        mu_hat = self.predict_mu(observation)
        seed = int(observation["probe_seed"]) % (2**31 - 1)
        return plan_cem(mu_hat, float(observation["alpha"]),
                        float(observation["s_star"]), seed=seed)


def load_policy() -> TractionProgramPolicy:
    here = os.path.dirname(os.path.abspath(__file__))
    payload = joblib.load(os.path.join(here, "policy.pt"))
    return TractionProgramPolicy(payload["members"])
